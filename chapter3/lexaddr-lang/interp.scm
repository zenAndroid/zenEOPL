(module interp (lib "eopl.ss" "eopl")
  ...
  (trace-define value-of-translation
    (lambda (pgm)
      (cases program pgm
        (a-program (exp1)
    ...
    (lambda (exp nameless-env)
      (cases expression exp
        ...
        (cond-exp (expressions-to-eval corresponding-consequents)
                  (cond-value expressions-to-eval corresponding-consequents nameless-env))
  ...
  (define (cond-value conds conseqs nameless-env)
    (if (null? conds)
        (eopl:error "Cond expression empty")
        (let ((head-cond-exp (translation-of (car conds) nameless-env))
              (head-consequent-exp (translation-of (car conseqs) nameless-env)))
          (let ((head-val (value-of head-cond-exp nameless-env)))
            (if (expval->bool head-val)
                (value-of head-consequent-exp nameless-env)
                (cond-value (cdr conds) (cdr conseqs) nameless-env))))))
